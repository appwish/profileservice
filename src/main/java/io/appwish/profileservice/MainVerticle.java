package io.appwish.profileservice;
...
import io.appwish.profileservice.repository.impl.PostgresProfileRepository;
import io.appwish.profileservice.repository.impl.Query;
import io.appwish.profileservice.verticle.DatabaseVerticle;
import io.appwish.profileservice.verticle.GrpcVerticle;
...
public class MainVerticle extends AbstractVerticle {
  ...
  public void start(final Promise<Void> startPromise) {
    ...
    retriever.getConfig(event -> {
      ...
      final ProfileRepository repository = new PostgresProfileRepository(client);
      final EventBusConfigurer util = new EventBusConfigurer(vertx.eventBus());

      util.registerCodecs();

      // TODO It's here just for development purposes
      client.preparedQuery(Query.CREATE_PROFILE_TABLE.sql(), query -> { });
  ...
  private Future<Void> deployDatabaseVerticle(final ProfileRepository repository) {
    final Promise<Void> promise = Promise.promise();
